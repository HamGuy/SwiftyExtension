...
public extension JKPOP where Base: ExpressibleByStringLiteral {
    ...
    #elseif canImport(AppKit)
    func setHtmlAttributedString(font: NSFont? = NSFont.systemFont(ofSize: 16), lineSpacing: CGFloat? = 10) -> NSMutableAttributedString {
        guard let string = baseString else {
            return NSMutableAttributedString(string: "")
        }
        var htmlString: NSMutableAttributedString? = nil
        do {
            ...
            print("HTML 解析错误: \(error.localizedDescription)")
        }
        // 设置富文本字的大小
        if let font = font {
            htmlString?.addAttributes([
...
public extension JKPOP where Base: ExpressibleByStringLiteral {
    ...
    func toViewController() -> NSViewController? {
        ...
        // 2.通过类创建对象
        // 2.1、将AnyClass 转化为指定的类
        guard let vcClass = trueClass as? NSViewController.Type else {
            return nil
        }
        // 2.2、通过class创建对象
        let vc = vcClass.init() 
    ...
    func toCGRect() -> CGRect? {
        ...
        if content.jk.predicateValue(rgex: pattern1) {
            return nil //NSCoder.cgRect(for: content)
        }
        // 3.2、正则判断 "(0,20,30,40)"
...
extension JKPOP where Base: ExpressibleByStringLiteral {
    ...
    #elseif canImport(AppKit)
    public func rectSize(font: NSFont, size: CGSize) -> CGSize {
        guard let str = base as? String else {
            return CGSize(width: 0, height: 0)
        }
        let attributes = [NSAttributedString.Key.font: font]
    ...
    #elseif canImport(AppKit)
    public func rectHeight(font: NSFont, size: CGSize) -> CGFloat {
        return rectSize(font: font, size: size).height
    }
    #endif
    ...
    #elseif canImport(AppKit)
    public func rectWidth(font: NSFont, size: CGSize) -> CGFloat {
        return rectSize(font: font, size: size).width
    }
    #endif
    ...
    #elseif canImport(AppKit)
    public func singleLineSize(font: NSFont) -> CGSize? {
        guard let string = baseString else {
            return nil
        }
        let attrs = [NSAttributedString.Key.font: font]
    ...
    #elseif canImport(AppKit)
    public func singleLineWidth(font: NSFont) -> CGFloat? {
        guard let string = baseString else {
            return nil
        }
        let attrs = [NSAttributedString.Key.font: font]
    ...
    #elseif canImport(AppKit)
    public func singleLineHeight(font: NSFont) -> CGFloat? {
        guard let string = baseString else {
            return nil
        }
        let attrs = [NSAttributedString.Key.font: font]
    ...
    #elseif canImport(AppKit)
    public func sizeAccording(width: CGFloat, height: CGFloat = CGFloat(MAXFLOAT), font: NSFont) -> CGSize? {
        guard let string = baseString, !self.isBlank else { return CGSize(width: 0, height: 0) }
        let rect = CGRect(x: 0, y: 0, width: width, height: height)
        let label = NSTextField(frame: rect).font(font).text(string)
        return label.sizeThatFits(rect.size)
    }
    #endif
    ...
    #elseif canImport(AppKit)
    public func widthAccording(width: CGFloat, height: CGFloat = CGFloat(MAXFLOAT), font: NSFont) -> CGFloat {
        guard let string = baseString, !self.isBlank else { return 0 }
        let rect = CGRect(x: 0, y: 0, width: width, height: height)
        let label = NSTextField(frame: rect).font(font).text(string)
        return label.sizeThatFits(rect.size).width
    ...
    #elseif canImport(AppKit)
    public func heightAccording(width: CGFloat, height: CGFloat = CGFloat(MAXFLOAT), font: NSFont) -> CGFloat {
        guard let string = baseString, !self.isBlank else { return 0 }
        let rect = CGRect(x: 0, y: 0, width: width, height: height)
        let label = NSTextField(frame: rect).font(font).text(string)
        return label.sizeThatFits(rect.size).height
    ...
    #elseif canImport(AppKit)
    public func sizeAccording(width: CGFloat, height: CGFloat = CGFloat(MAXFLOAT), font: NSFont, lineSpacing: CGFloat) -> CGSize {
        guard let string = baseString, !self.isBlank else { return CGSize(width: 0, height: 0) }
        let rect = CGRect(x: 0, y: 0, width: width, height: CGFloat(MAXFLOAT))
        let label = NSTextField(frame: rect).font(font).text(string)
        let attrStr = NSMutableAttributedString(string: string)
    ...
    #elseif canImport(AppKit)
    public func widthAccording(width: CGFloat, height: CGFloat = CGFloat(MAXFLOAT), font: NSFont, lineSpacing: CGFloat) -> CGFloat {
        guard let string = baseString, !self.isBlank else { return 0 }
        let rect = CGRect(x: 0, y: 0, width: width, height: height)
        let label = NSTextField(frame: rect).font(font).text(string)
        let attrStr = NSMutableAttributedString(string: string)
    ...
    #elseif canImport(AppKit)
    public func heightAccording(width: CGFloat, height: CGFloat = CGFloat(MAXFLOAT), font: NSFont, lineSpacing: CGFloat) -> CGFloat {
        guard let string = baseString, !self.isBlank else { return 0 }
        let rect = CGRect(x: 0, y: 0, width: width, height: height)
        let label = NSTextField(frame: rect).font(font).text(string)
        let attrStr = NSMutableAttributedString(string: string)
...
extension JKPOP where Base: ExpressibleByStringLiteral {
    ...
    #elseif canImport(AppKit)
    public func stringWithHighLightSubstring(keyword: String, font: NSFont, normalColor: NSColor, keywordCololor: NSColor, isSplit: Bool = false, options: NSRegularExpression.Options = []) -> NSMutableAttributedString {
        guard let totalString = base as? String else {
            return NSMutableAttributedString(string: "")
        }
        let attributedString = NSMutableAttributedString(string: totalString)
    ...
    #elseif canImport(AppKit)
    public func highlightKeywords(keywords: [String], normalColor: NSColor, highlightColor: NSColor, specialKeywords: [String] = [], specialColor: NSColor? = nil, isCaseSensitive: Bool = false, isMatchesWholeWords: Bool = false) -> NSAttributedString {
        guard let text = base as? String else {
            return NSMutableAttributedString(string: "")
        }
        // 1、创建富文本
...
public extension JKPOP where Base: ExpressibleByStringLiteral {
    ...
    func font(_ font: CGFloat) -> NSMutableAttributedString {
        ...
        #elseif canImport(AppKit)
        let attributedText = NSMutableAttributedString(string: baseString, attributes: [NSAttributedString.Key.font: NSFont.systemFont(ofSize: font)])
        #endif
        return attributedText
    }
    
    // MARK: 14.3、String 添加 UIFont 后转 NSMutableAttributedString
