...
public extension JKPOP where Base: FileManager {
    ...
    #elseif canImport(AppKit)
    static func getLocalVideoImage(videoPath: String, preferredTrackTransform: Bool = true) -> NSImage? {
        //  获取截图
        let videoImage = getVideoImage(videoUrlSouceType: .local, path: videoPath, seconds: 1, preferredTimescale: 10, maximumSize: nil, preferredTrackTransform: preferredTrackTransform)
        return videoImage
    }
    ...
    #elseif canImport(AppKit)
    static func getLocalVideoImages(videoPaths: [String], preferredTrackTransform: Bool = true) -> [NSImage?] {
        //  获取截图
        var allImageArray: [NSImage?] = [] 
        for path in videoPaths {
            let videoImage = getVideoImage(videoUrlSouceType: .local, path: path, seconds: 1, preferredTimescale: 10, maximumSize: nil, preferredTrackTransform: preferredTrackTransform)
            allImageArray.append(videoImage)
        }
    ...
    #elseif canImport(AppKit)
    static func getServerVideoImage(videoPath: String, videoImage: @escaping (NSImage?) -> Void, preferredTrackTransform: Bool = true) {
        //异步获取网络视频缩略图，由于网络请求比较耗时，所以我们把获取在线视频的相关代码写在异步线程里
        DispatchQueue.global().async {
            //  获取截图
            let image = getVideoImage(videoUrlSouceType: .server, path: videoPath, seconds: 1, preferredTimescale: 10, maximumSize: nil, preferredTrackTransform: preferredTrackTransform)
    ...
    #elseif canImport(AppKit)
    static func getServerVideoImages(videoPaths: [String], videoImages: @escaping ([NSImage?]) -> Void, preferredTrackTransform: Bool = true) {
        //异步获取网络视频缩略图，由于网络请求比较耗时，所以我们把获取在线视频的相关代码写在异步线程里
        DispatchQueue.global().async {
            //  获取截图
            var allImageArray: [NSImage?] = []
    ...
    #elseif canImport(AppKit)
    private static func getVideoImage(videoUrlSouceType: JKVideoUrlType = .local, path: String, seconds: Double = 1, preferredTimescale: CMTimeScale = 10, maximumSize: CGSize?, preferredTrackTransform: Bool = true) -> NSImage? {
        var videoURL: URL?
        ...
        let cmTime = CMTime(seconds: seconds, preferredTimescale: preferredTimescale)
        if let cgImg = try? imageGenerator.copyCGImage(at: cmTime, actualTime: nil) {
            let img = NSImage(cgImage: cgImg, size: CGSize(width: cgImg.width, height: cgImg.height))
            return img
        } else {
            JKPrint("获取缩略图失败")
            return nil
